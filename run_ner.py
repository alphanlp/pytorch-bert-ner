...
class ChineseNerProcessor(DataProcessor):
    """Processor for the 2014 People's Daily data set."""
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        input_mask = [mask1 + mask2 for mask1, mask2 in zip(input_mask, x_input_mask)] # add label X mask
...
def main():
    ...
    processors = {"ner": NerProcessor,
                  "chinese_ner": ChineseNerProcessor}
    ...
    num_labels = len(label_list) + 1 # +1 for PAD label
